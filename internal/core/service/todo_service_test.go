...
func (s *TodoServiceTestSuite) Test_Get_All_Should_Return_Todo_List_When_There_Are_Items() {
	...
	assert.NotEqual(s.T(), todo.ZeroID, todos[0].ID)
...
func (s *TodoServiceTestSuite) Test_Find_Should_Return_Todo_When_Given_Todo_Id() {
	...
	s.MockTodoRepository.MockGetByID = func(ctx context.Context, id todo.ID) (todo.Todo, error) {
		return todo.Todo{
			ID:          todoID,
			Title:       "Task 1",
			Description: "Task 1 Desc",
	...
	assert.Nil(s.T(), err)
	assert.NotNil(s.T(), t)
	assert.NotEqual(s.T(), t.ID, todo.ZeroID)
...
func (s *TodoServiceTestSuite) Test_Create_Todo_Should_Return_Created_Todo_Id_When_Given_Todo_Entity() {
	...
	s.MockTodoRepository.MockInsert = func(ctx context.Context, todo todo.Todo) (todo.ID, error) {
		return newID, nil
	}
	...
	// Then
	assert.Nil(s.T(), err)
	assert.NotEqual(s.T(), modelID, todo.ZeroID)
...
func (s *TodoServiceTestSuite) Test_Create_Todo_Should_Return_Error_When_Connection_Error_Occurs() {
	...
	s.MockTodoRepository.MockInsert = func(ctx context.Context, todo todo.Todo) (todo.ID, error) {
		return todo.ZeroID, errorMessage
	}
	...
	assert.Equal(s.T(), todo.ZeroID, id)
}
...
func (s *TodoServiceTestSuite) Test_Delete_Should_Return_No_Error_When_Given_Todo_Id() {
	//Given
	newID := s.MockIDGenerator.NewID()
	s.MockTodoRepository.MockDelete = func(ctx context.Context, id todo.ID) error {
		return nil
	}
